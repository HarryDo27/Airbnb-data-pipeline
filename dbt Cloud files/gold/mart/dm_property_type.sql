...
AGGREGATED AS (
    SELECT
        ...
        MIN(CASE WHEN HAS_AVAILABILITY = TRUE THEN PRICE ELSE NULL END) AS MIN_PRICE,
        MAX(CASE WHEN HAS_AVAILABILITY = TRUE THEN PRICE ELSE NULL END) AS MAX_PRICE,
        PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY PRICE) FILTER (WHERE HAS_AVAILABILITY = TRUE) AS MEDIAN_PRICE,
        AVG(CASE WHEN HAS_AVAILABILITY = TRUE THEN PRICE ELSE NULL END) AS AVG_PRICE,
...
CHANGES AS (
    SELECT
        ...
        MONTH_YEAR,
        ACTIVE_LISTINGS,
        LAG(ACTIVE_LISTINGS) OVER (PARTITION BY PROPERTY_TYPE, ROOM_TYPE, ACCOMMODATES ORDER BY MONTH_YEAR) AS PREVIOUS_ACTIVE_COUNT,
        TOTAL_LISTINGS,
